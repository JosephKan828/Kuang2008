...
const OUTPATH  = WORKPATH * "output/Full/"
const FIGPATH  = ROOT * "figures/Full/"
...
init[1, 1:end] = randn(ComplexF64, Nk).*(1 + 1im) * 0.0001
init[2, 1:end] = randn(ComplexF64, Nk).*(1 + 1im) * 0.0001
init[3, 1:end] = randn(ComplexF64, Nk).*(1 + 1im) * 0.1
init[4, 1:end] = randn(ComplexF64, Nk).*(1 + 1im) * 0.1
init[5, 1:end] = randn(ComplexF64, Nk).*(1 + 1im) * 0.1
init[6, 1:end] = randn(ComplexF64, Nk).*(1 + 1im) * 1
...
# 1. Run full model
# ============================================
run_full_model!(state_vec, t, k, init; params=params, mode=:full)
