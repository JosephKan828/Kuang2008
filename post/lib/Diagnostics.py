...
def modal_growth_rate(optrs: np.ndarray, k: np.ndarray) -> np.ndarray:
...
def phase_speed(optrs: np.ndarray, k: np.ndarray) -> np.ndarray:
...
def convert_state(state: np.ndarray) -> np.ndarray:
    ...
    state_new = np.concatenate([state[:4], J1, J2], axis=0).astype(
